...
**v0.10.4** — acoustic speaker separation is switched off for now, and speaker names now arrive after short meetings as well. While the service is off, the options for splitting the meeting channel into individual speakers are hidden, and the app no longer uploads the meeting audio after every recording for no purpose. You and the meeting are still told apart as before. Speaker names (Pro) are now suggested when you stop a recording shorter than a minute and a half, and they appear in the transcript right away.

**v0.10.3** — upgrading to Pro is tied to your account wherever you start it. The upgrade button in the trial banner opened the payment page without linking it to your account, so a purchase made there could fail to activate Pro; every upgrade button in the app now goes through the same flow, and it always knows who is buying. If you already have a subscription — or have just paid and Pro has not shown up in the app yet — you now get told so instead of being sent to a second payment, so you can no longer be charged twice by accident. The upgrade buttons show that the payment page is opening, and say plainly when you need to sign in or have no internet connection. The price in the upgrade dialog also read "excluding VAT"; Pro costs 199 kr a month including VAT, as on sagt.ai.
